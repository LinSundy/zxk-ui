<template>
  <div class="audio">
    <div class="view_container">
      <div class="progress" ref="progress" @click="skip">
        <div class="on" :style="{'width': `${progress * 100}%`}">
          ...
          <div class="drag-btn" @touchstart="drag" @touchmove.prevent="move" @touchend="up" v-else></div>
        </div>
      </div>
    ...
    <div class="operation_container">
      ...
      <span class="iconfont icon-previous prev" @click="prev" v-if="isMultiple"></span>
      <span :class="`play-btn ${status ? 'icon-pause' : 'icon-play'}`" @click="toggle"></span>
      <span class="iconfont icon-previous next" @click="next" v-if="isMultiple"></span>
      <div class="volume-container">
        <span :class="`iconfont icon-voice volume ${!isMultiple && 'single'}`" @click="volumeToggle"></span>
        <div class="volume-progress" ref="volumeProgress">
          <div class="on" :style="{width: `${isMute ? '0' : volumeProgress * 100}%`}">
            <span class="volume-drag" @mousedown="volumeDrag" v-if="!isMobile"></span>
            <span class="volume-drag" @touchstart="volumeDrag" @touchmove.prevent="volumeMove" @touchend="volumeUp" v-else></span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    volumeToggle() {
      ...
      this.isMute ? this.ap.volume = 0 : this.ap.volume = this.volumeProgress
    },
    ...
    move(e) {
      ...
      let i = (mouseClient - this.$refs.progress.getBoundingClientRect().left) / this.progressNodeWidth;
      i = i > 0 ? i : 0;
      i = 1 > i ? i : 1;
      this.progress = i;
      this.changeCurrentTime()
    ...
    volumeMove(e) {
      let mouseClient = e.touches && e.touches[0].clientX || e.clientX
      let i = (mouseClient - this.$refs.volumeProgress.getBoundingClientRect().left) / this.volumeProgressNodeWidth;
      i = i > 0 ? i : 0;
      i = 1 > i ? i : 1;
    ...
    initAudio() {
      if (this.isMultiple) {
        // 多个音频初始化
        this.ap = new Audio()
        if(!(this.musics && this.musics[this.currentAudioIndex] && this.musics[this.currentAudioIndex].url)) {
          return console.error('音频的url不允许为空')
        }
